...
const Game = ({ onGameEnd, challengeSyllable, gameplayStart, gameplayStop, isAdPlaying }: GameProps) => {
  ...
  // Ensure that the dictionary is loaded when the game starts.
  useEffect(() => {
  ...
  // Synchronise refs with state to avoid stale closures
  useEffect(() => {
  ...
  useEffect(() => {
    // Only when the dictionary is ready, start the game.
    if (isDictReady) {
      gameplayStart();
  ...
  useEffect(() => {
    // Timer starts only if dictionary is ready
  ...
  const handleTimeUp = () => {
    ...
    // Use refs instead of state to get current values (avoid stale closures)
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (validation.valid) {
      ...
      const newWord: FoundWord = {
        ...
        comboMultiplier: newCombo.multiplier > 1 ? newCombo.multiplier : 1,
        timestamp,
